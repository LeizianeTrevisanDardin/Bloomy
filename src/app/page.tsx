...
function StatisticsPanel() {
  ...
  return (
    <div className="min-h-[270px] rounded-2xl border border-white/[0.08] bg-gradient-to-br from-white/[0.055] to-white/[0.025] p-5">
      ...
      {error && (
        <p className="mt-4 rounded-xl border border-red-400/15 bg-red-500/5 px-3 py-2 text-xs text-red-300">
          {error}
        </p>
      )}
      ...
      <div className="mt-5 grid grid-cols-2 gap-3 lg:grid-cols-4">
        {loading ? (
          [0, 1, 2, 3].map((item) => (
            <div
              key={item}
              className="h-[112px] animate-pulse rounded-xl bg-black/20"
            />
          ))
        ) : (
          <>
            ...
            <Stat
              icon="🌱"
              value={`${statistics.habitWeeklyRate}%`}
      ...
      {!loading && (
        <div className="mt-4 grid grid-cols-3 gap-2 border-t border-white/[0.06] pt-4 text-center">
          <MiniStat
            value={`${statistics.habitsCompletedToday}/${statistics.activeHabits}`}
            label="Habits today"
          />
          <MiniStat
          ...
          />
          <MiniStat
            value={`${statistics.completedGoals}/${statistics.totalGoals}`}
            label="Goals"
          />
        </div>
      )}
